...
function Category({ module }) {
    ...
    const fetchCategories = async () => {
        try {
            const categoryResponse = await fetchData(`/api/category`, { moduleCode: moduleName });
            const categoryData = await categoryResponse.data;
            const subCategoryResponse = await fetchData(`/api/subcategory`, { moduleCode: moduleName });
    ...
    return (
        <div>
            ...
            <Card title="Category Management">
                <Row gutter={[16, 16]}>
                    <Col span={6}>
                        <Button style={{ width: '100%' }} onClick={() => (true)}>Expense Category</Button>
                    </Col>
                    <Col span={6}>
                        <Button style={{ width: '100%' }} onClick={() => (true)}>Investment Category</Button>
                    </Col>
                    <Col span={6}>
                        <Button style={{ width: '100%' }} onClick={() => (true)}>Saving Category</Button>
                    </Col>
                    <Col span={6}>
                        <Button style={{ width: '100%' }} onClick={() => openCategoryModal()}>Add {moduleName}</Button>
                    </Col>
                </Row>
                <Table
                    columns={columns}
